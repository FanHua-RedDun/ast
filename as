...
// libcurl回调处理 - 强制处理方案
size_t process_data(void *buffer, const size_t size, const size_t nmemb, void *user_p) {
    const auto fp = static_cast<FILE *>(user_p);
    const size_t return_size = fwrite(buffer, size, nmemb, fp);
    // cout << static_cast<char *>(buffer) << endl;
    return return_size;
}
...
public:
    ...
    [[nodiscard]] auto run() const {
        ...
        // ===========================================================================================
        // libcurl回调处理 - 强制处理方案
        curl_easy_setopt(_curl, CURLOPT_WRITEFUNCTION, &process_data);
        // 打开文件 》 写入数据 - 强制处理方案
        FILE *fp = fopen("html.html", "ab+");
        // libcurl回调处理 - 强制处理方案
        ...
        std::ifstream file("html.html"); // 打开文件
        ...
        std::string content;
        std::getline(file, content); // 读取文件中的第一行并存储在content变量中
        ...
        // if (ass ifs("html.html", ios::in); ifs)
        // {
